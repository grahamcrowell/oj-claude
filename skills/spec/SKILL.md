...
- **Location**: file each doc **by its type**, since this skill's three artifacts map one-to-one onto typed keys. Ask `oj-helper resolve-path requirements|design|plan`, adding `--node <relpath>` to name the node that owns the subject (`oj-helper resolve-path design --node billing/invoicing`), and write to the resolved file. If the key exits 3, the project has no per-type filing surface — fall back to the project's `program/` artifacts area (e.g. `docs/program/<subject>-<artifact>.md`), or to `oj-helper resolve-path artifacts` when writing into the OpenJunto state tree. **A review is not a design**: this skill authors new intent, so its output belongs at the node, never in a dated history folder. Full procedure: `${CLAUDE_PLUGIN_ROOT}/reference/file-patterns.md` § Filing Rule.
