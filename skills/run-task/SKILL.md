...
- **Phase 1 — Stakeholder Analysis**: The mandatory Product + Distinguished pair runs **inline** as PERSPECTIVE blocks. Spawn a stakeholder agent only when it would genuinely read different files than the manager already has, capped at three. Analysis only; no implementation.
